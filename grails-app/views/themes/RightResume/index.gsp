...
<head>
  ...
  <asset:stylesheet src="RightResume/all.min.css"/>
  %{--    <link href="css/bootstrap.min.css?ver=1.2.0" rel="stylesheet">--}%
  <asset:stylesheet src="RightResume/bootstrap.min.css"/>
  %{--    <link href="css/aos.css?ver=1.2.0" rel="stylesheet">--}%
  <asset:stylesheet src="RightResume/aos.css"/>
  %{--    <link href="css/main.css?ver=1.2.0" rel="stylesheet">--}%
  <asset:stylesheet src="RightResume/main.css"/>
  <noscript>
    <style type="text/css">
    [data-aos] {
      opacity: 1 !important;
...
<header class="d-print-none">
  <div class="container text-center text-lg-left">
    <div class="py-3 clearfix">
      <h1 class="site-title mb-0">${this.appUser?.name} ${this.appUser?.surname}</h1>
      %{--          <div class="site-nav">--}%
      %{--            <nav role="navigation">--}%
        ...
        <div class="row">
          <div class="col-lg-4 col-md-5">
            <div class="avatar hover-effect bg-white shadow-sm p-1">
              <img src="<g:createLink controller="appUser" action="featuredImage" id="${this.appUser?.id}"/>" width="200" height="200"/>
              %{--          <asset:image width="200" height="200" src="RightResume/avatar.jpg" alt=""/>--}%

            </div>
          </div>
          <div class="col-lg-8 col-md-7 text-center text-md-start">
            <h2 class="h1 mt-2" data-aos="fade-left" data-aos-delay="0">${this.appUser?.name} ${this.appUser?.surname}</h2>
            <p data-aos="fade-left" data-aos-delay="100">${this.appUser?.headline}</p>
        ...
        <div class="row">
          ...
          <div class="col-md-5 offset-md-1">
            <div class="row mt-2">
              ...
              <div class="col-sm-8">
                <div class="pb-1 text-secondary">${this.appUser?.email}</div>
              </div>
              <div class="col-sm-4">
                <div class="pb-1">Phone</div>
              </div>
              <div class="col-sm-8">
                <div class="pb-1 text-secondary">${this.appUser?.phoneNumber}</div>
              </div>
              <div class="col-sm-4">
                <div class="pb-1">Address</div>
              </div>
              <div class="col-sm-8">
                <div class="pb-1 text-secondary">${this.appUser?.address}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <g:each in="${this.appUser?.skillCategoryList}" var="skillCategory" status="i">
            <div class="col-md-6">
              ...
              <g:each in="${skillCategory?.skillList}" var="skillItem">
                <div class="mb-2"><span>${skillItem?.skillName}</span>
                  <g:if test="${skillItem?.level > 0}">
                    <div class="progress my-1">
                      <div class="progress-bar" role="progressbar" data-aos="zoom-in-right" data-aos-delay="${i*200+100}" data-aos-anchor=".skills-section"
                           style="width: ${skillItem?.level * 20}%; background-color: rgb(74,137,220);" aria-valuenow="${skillItem?.level * 20}" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                  </g:if>
                </div>
              </g:each>
        ...
        <div class="timeline">
          ...
          <g:each in="${this.appUser?.workExpList}" var="workExpItem" status="i">
            <div class="timeline-card timeline-card-success card shadow-sm">
              <div class="card-body">
                <div class="h5 mb-1">${workExpItem?.title} <span class="text-muted h6">at ${workExpItem?.employerName}</span></div>
                <div class="text-muted text-small mb-2"><g:formatDate format="MMM yyyy" date="${workExpItem?.startDate}"/> - <g:if
                        test="${workExpItem?.current}">Present</g:if><g:else><g:formatDate format="MMM yyyy" date="${workExpItem?.endDate}"/></g:else></div>
                <div>${workExpItem?.description}</div>
              </div>
            </div>
          </g:each>
      ...
      <div class="education-section px-3 px-lg-4 pb-4">
        ...
        <div class="timeline">
          <g:each in="${this.appUser?.educationList}">
            <div class="timeline-card timeline-card-success card shadow-sm">
              <div class="card-body">
                <div class="h5 mb-1">${it?.sector} <span class="text-muted h6">from ${it?.name}</span></div>
                <div class="text-muted text-small mb-2"><g:formatDate format="yyy" date="${it?.startDate}"/> - <g:if
                        test="${it?.current}">Present</g:if><g:else><g:formatDate
                        format="yyy" date="${it?.endDate}"/></g:else></div>
                %{--                <div>Leverage agile frameworks to provide a robust synopsis for high level overviews. Iterative approaches to corporate strategy foster collaborative thinking to further the overall value proposition.</div>--}%
              </div>
            </div>
          </g:each>
        ...
        <div class="row">
          ...
          <div class="col">
            <div class="mt-2">
              ...
              <div class="pb-2 text-secondary">${this.appUser?.address}</div>
            </div>
          </div>
          <div class="col">
            <div class="mt-2">
              ...
              <div class="pb-2 text-secondary">${this.appUser?.phoneNumber}</div>
            </div>
          </div>
          <div class="col">
            <div class="mt-2">
              <h3 class="h6">Email</h3>
              <div class="pb-2 text-secondary">${this.appUser?.email}</div>
            </div>
          </div>
        </div>
...
<footer class="pt-4 pb-4 text-muted text-center d-print-none">
  <div class="container">
    <div class="my-3">
      <div class="h4">${this.appUser?.name} ${this.appUser?.surname}</div>
    ...
    <div class="text-small">
      <div class="mb-1">© ${Calendar.getInstance().get(Calendar.YEAR)} IsLinked.me</div>
      <div>Design - <a href="https://templateflip.com/" target="_blank">TemplateFlip</a></div>
    </div>
  </div>
</footer>
%{--    <script src="scripts/bootstrap.bundle.min.js?ver=1.2.0"></script>--}%
<asset:javascript src="RightResume/bootstrap.bundle.min.js"/>
%{--    <script src="scripts/aos.js?ver=1.2.0"></script>--}%
<asset:javascript src="RightResume/aos.js"/>
%{--    <script src="scripts/main.js?ver=1.2.0"></script>--}%
<asset:javascript src="RightResume/main.js"/>
